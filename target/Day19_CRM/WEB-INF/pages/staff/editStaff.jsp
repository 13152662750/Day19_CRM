...
<form action="${pageContext.request.contextPath}/staff/editStaff" method="post">
	...
	<table width="88%" border="0" class="emp_table" style="width:80%;">
	 ...
	 <tr>
	    ...
	    <td>性别：</td>
	    <td>
	    	<input type="radio" name="gender" ${staff.gender=="男"?"checked='checked'":""} value="男"/>男
	    	<input type="radio" name="gender" ${staff.gender=="女"?"checked='checked'":""} value="女"/>女
	    </td>
	  </tr>
	 <tr>
	    <td width="10%">所属部门：</td>
	    <td width="20%">
	    	<select name="crmPost.crmDepartment.depid"  onchange="changePost(this)">
				...
				<c:forEach items="${depts}" var="dept">
					<option value="${dept.depid}" ${dept.depid==staff.crmPost.crmDepartment.depid?"selected='selected'":""}>${dept.depname}</option>
				</c:forEach>
